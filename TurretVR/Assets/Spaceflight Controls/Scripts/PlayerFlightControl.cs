...
public class PlayerFlightControl : MonoBehaviour
{
    ...
    public Forge3D.F3DTurret[] turrets = new Forge3D.F3DTurret[2];
    ...
    Slider speedSlider;
    int thrust = 0;
    float currentSpeedSliderValue = 0.0f;
    ...
    void FixedUpdate()
    {
        ...
        if (thrust > 0)
        {
            ...
            slow_Active = false;
            currentMag = Mathf.Lerp(currentMag, afterburner_speed * currentSpeedSliderValue, thrust_transition_speed * Time.deltaTime);
        ...
        else if (thrust < 0)
        {
            ...
            currentMag = Mathf.Lerp(currentMag, slow_speed * currentSpeedSliderValue, thrust_transition_speed * Time.deltaTime);
        ...
        { //Otherwise, hold normal speed.
            slow_Active = false;
            afterburner_Active = false;
            currentMag = Mathf.Lerp(currentMag, currentMag, thrust_transition_speed * Time.deltaTime);
        ...
        GetComponent<Rigidbody>().velocity = -transform.forward * currentMag; //Apply speed multiply on -1 because of incorrect model

        if (use_banking)
            updateBanking(); //Calculate banking.

        var targetPosition = playerCamera.ScreenToWorldPoint(
            new Vector3(
                CustomPointer.pointerPosition.x,
                CustomPointer.pointerPosition.y,
                3000));
    ...
    {
        if (currentSpeedSliderValue > speedSlider.value) thrust = -1;
        else if (currentSpeedSliderValue < speedSlider.value) thrust = 1;
        else thrust = 0;
        currentSpeedSliderValue = speedSlider.value;
    }
}